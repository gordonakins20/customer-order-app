...
function renderOrders() {
  ...
  $('ordersBody').innerHTML = state.orders.map(o => {
    const items = o.items.map(i => `${i.quantity}× ${escapeHtml(i.product.name)}`).join('<br>');
    const options = statuses.map(s => `<option value="${s}" ${s === o.status ? 'selected' : ''}>${s}</option>`).join('');
    return `<tr>
      <td>#${o.id}</td><td>${escapeHtml(o.customer.name)}</td><td>${items}</td><td>${money(o.total)}</td>
      <td><select onchange="changeStatus(${o.id}, this.value)">${options}</select></td>
      <td>${dateTime(o.createdAt)}</td>
      <td><button class="danger small" onclick="deleteOrder(${o.id})">Delete</button></td>
    </tr>`;
...
window.changeStatus = async (id, status) => {
  try { await api(`/api/orders/${id}/status`, { method:'PATCH', body: JSON.stringify({ status }) }); flash('Order status updated.'); await loadAll(); }
  catch (err) { flash(err.message, true); await loadAll(); }
};
